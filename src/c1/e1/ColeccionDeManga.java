...
public class ColeccionDeManga {
	...
	/**
	 * Se construya a partir de la cantidad de tomos que tiene.
	 * Agregue un tomo, indicando nombre y la cantidad de páginas que contiene. Como toda colección de Manga, se agrega de derecha a izquierda.
	 * Devuelva la cantidad de páginas de un tomo dado su nombre.
	 * Cuente la cantidad de tomos en la colección.
	 * Calcule el tiempo que se tardará en llegar al tomo en la posición `x`, asumiendo que se lee una página por minuto y debo leer todos los tomos a la derecha del que deseo leer, antes de llegar a dicho tomo. ¡No quiero perderme ni un capítulo!
	 */
	...
	public int obtenerPaginasParaTomo(String titulo) {
		for (Tomo tomo : tomos) {
			if (tomo.getTitulo().equals(titulo)) {
				return tomo.getPaginas();
			}
		}
